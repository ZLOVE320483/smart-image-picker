...
abstract class BaseSystemPickerView: androidx.fragment.app.Fragment() {
    ...
    protected var publishSubject = PublishSubject.create<Result>()

    private val canceledSubject = PublishSubject.create<Int>()
    ...
    private fun onImagePicked(uri: Uri?) {
        uri?.let {
            publishSubject.onNext(parseResultNoExtraData(it))
        }
